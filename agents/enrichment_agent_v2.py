...
Topic classification cascade (in priority order):
  ...
  Layer 5: Ollama zero-shot             — last resort for uncategorized
...
def _classificar_por_temas_camara(temas_api: list) -> Optional[str]:
    """
    Layer 1: Maps Câmara official themes to internal categories.
    Returns the first matching category or None if no match found.
    """
    for tema in temas_api:
        nome = (tema.get("nome") or "").lower().strip()
...
# =============================================================================
# LAYER 5 — OLLAMA ZERO-SHOT (last resort)
# =============================================================================
...
def _classificar_zero_shot_ollama(texto: str) -> Optional[str]:
    """
    Layer 5: Uses local Ollama to classify topic via zero-shot prompt.
    Slowest layer — only called when all previous layers return None.
    """
    from config import OLLAMA_BASE_URL, OLLAMA_MODEL
    import json

    labels_str = "\n".join(f"- {l}" for l in CANDIDATE_LABELS)
    prompt = f"""Você é um classificador legislativo. Leia o texto abaixo e escolha UMA categoria da lista.
...
CATEGORIA:"""
    ...
    try:
        ...
        if not categoria:
            ...
            return None
        # Find closest match in candidate labels
        ...
        logger.error(f"Ollama zero-shot error: {e}")
...
def classificar_votacao_cascata(
    ...
    # Layer 4: short ementa — available for most votacoes
    if ementa and len(ementa) > 10:
        cat = _inferir_tema_bertopic(ementa)
        if cat and cat != "uncategorized":
            return cat, "ementa_bertopic"

    # Layer 5: Ollama zero-shot — slowest, last resort
    texto_fallback = " ".join(filter(None, [ementa_detalhada, ementa, descricao]))
    if texto_fallback and len(texto_fallback) > 20:
        cat = _classificar_zero_shot_ollama(texto_fallback)
        if cat:
            return cat, "ollama_zeroshot"
...
def _enriquecer_uma(votacao_id: str, forcar: bool = False) -> str:
    ...
    if id_prop:
        try:
            ...
            # For procedural propositions (DTQ, REQ, etc.), try the principal proposition
            sigla = prop_dados.get("siglaTipo", "")
            TIPOS_PROCEDIMENTAIS = {"DTQ", "REQ", "RPD", "EMA", "RCP", "REL"}
            if sigla in TIPOS_PROCEDIMENTAIS and not texto_integral:
                ...
                if uri_principal:
                    try:
                        ...
                        # Also inherit themes and keywords from principal proposition
                        if not temas_api:
    ...
    # Step 5: cascade topic classification — independent of text availability
    ...
    # Step 7: run NLP pipeline (keywords + summary)
    ...
    # Override tema_inferido with the cascade result — more reliable than BERTopic alone
            ...
            rows = session.execute(text("""
                SELECT
                    CASE
                        ...
                        WHEN modelo_keywords LIKE '%ementa_detalhada%'       THEN 'Layer 3 — Ementa Detalhada'
                        WHEN modelo_keywords LIKE '%ementa_bertopic%'        THEN 'Layer 4 — Ementa BERTopic'
                        WHEN modelo_keywords LIKE '%ollama_zeroshot%'        THEN 'Layer 5 — Ollama Zero-shot'
                        ELSE 'Unknown'
                    END AS fonte,
                    COUNT(*) AS total,
                    ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (), 1) AS pct
